...
## The engine
    ...
    src/industriallib.py  the industrial techno module: the rumble, the
                      ...
                      the fabrication shop - `mill()`, a machine tool running
                      its own cycle (it bites, screeches while the edge is in
                      the metal, labours and exhales, and idles in between;
                      the cycle is in STEPS and must be EVEN so a drifting
                      machine still lands on an eighth), `pipe()`, a tube
...
| `punk_antenna_164.wav` | Post-punk / new wave in A minor, 3:25 - Manchester 1979 crossed with the German new wave, and the record where the rhythm guitar stopped being a patch. `riff()` renders each phrase as ONE take through ONE amp pass: strings ring into the next chord until the hand chokes them, restrikes re-excite strings that never stopped, the pick scrapes at every change and the amp hisses in the rests - measured, the old chord-per-segment wall falls to digital zero between chords and this take holds 0.52. Three different guitars at once: the humbucker wall, a single-coil `jangle()` on the edge of break-up through a chorus pedal, and a lead - plus a one-oscillator synth doubling the chorus tune, 1983-style. The bass carries the verses the way Peter Hook played, high on the neck; the chorus is the VI-VII-i anthem cadence; gang "oh"s on the last one |
...
| `industrial_walzwerk_168.wav` | Hard industrial techno in F Locrian, 6:06, at the fast end of the range and with no 303 anywhere - a rolling mill. Take the acid away and something else has to carry pitch for six minutes, so the melodic voice is the building: `girder()` is a beam the size of the hall, excited by friction rather than struck, gliding F1 to the tritone above it and back across forty seconds, which is a thing an object that size cannot do. The machines are `mill()` - a tool that bites, screeches while the edge is in the metal, labours and exhales, and idles in between - and three of them run at 6, 10 and 14 steps, so they agree with the bar and with each other once every 105 bars. Every cycle is EVEN, so every hit still lands on an eighth: a pitched ringing figure that never touches a beat is not heard as polymeter, it is heard as a second machine playing badly. Under the drops is `shear()`, the plate letting go - one oscillator split at 92 Hz, the weight clean and untouched, the 2nd and 3rd harmonics band-limited to 120-800 Hz so you can hear which note it is, and everything above that through a folder whose amount ramps across the phrase and a resonant filter moved by a lane that accelerates from a quarter to a thirty-second. It plays notes with real gaps in them and glides between them without ever retriggering, because a held pitch under a moving filter is a texture and what the ear reports about a texture in the sub is noise. Wherever it plays the kick's own clean sub steps almost all the way out and the offbeat bass stops entirely, so the drop changes the character of the low end rather than the level of it. The b5 is the argument: the root and the B a tritone above are the only two notes on the record, and the pipes, the girder and the stabs all play both. The offbeat kicks are a shorter, drier, low-passed voice rather than the same kick fired more often - eight hard-clipped kicks a bar overlap into one continuous rasp otherwise. Four holes, and the body spans 6.3 dB |
...
python3 src/track_antenna.py     # writes renders/punk_antenna_164.wav
...
python3 src/track_walzwerk.py    # writes renders/industrial_walzwerk_168.wav
