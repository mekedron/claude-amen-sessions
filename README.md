...
| `industrial_walzwerk_168.wav` | Hard industrial techno in F Locrian, 6:06, at the fast end of the range and with no 303 anywhere - a rolling mill. Take the acid away and something else has to carry pitch for six minutes, so the melodic voice is the building: `girder()` is a beam the size of the hall, excited by friction rather than struck, gliding F1 to the tritone above it and back across forty seconds, which is a thing an object that size cannot do. The machines are `mill()` - a tool that bites, screeches while the edge is in the metal, labours and exhales, and idles in between - and three of them run at 6, 10 and 14 steps, so they agree with the bar and with each other once every 105 bars. Every cycle is EVEN, so every hit still lands on an eighth: a pitched ringing figure that never touches a beat is not heard as polymeter, it is heard as a second machine playing badly. Under the drops is `shear()`, the plate letting go - one oscillator split at 92 Hz, the weight clean and untouched and everything above it through a folder whose amount ramps across the phrase and a resonant filter moved by a lane that accelerates from a quarter to a sixty-fourth without ever retriggering. Wherever it plays the kick's own sub steps almost all the way out, because two sines at 43 Hz with unrelated phases cancel - so the drop changes the character of the low end rather than the level of it. The b5 is the argument: the root and the B a tritone above are the only two notes on the record, and the pipes, the girder and the stabs all play both. Four one-beat holes, and the sections span 9.6 dB |
