"""BEZDNA - dark neurofunk, F minor, 174 BPM.

Nothing here is sampled. The kit is built from parts and the bass is designed
in three layers that never share a frequency: a clean mono sine under 110 Hz,
a reese or a growl above it, and a hard-synced screech on top.

The harmony is one note - F - with the Phrygian b2 (Gb) used as the only
outside colour, on the last bar of every four. That is as much harmony as this
genre wants; everything else that moves is timbre.

    intro     0-15    the room, before anything is in it
    approach  16-31   drums arrive filtered, the sub starts breathing
    build 1   32-39
    DROP 1    40-71   reese and growl, 32 bars, the riff stated
    breakdown 72-87   the only chords in the track
    build 2   88-95
    DROP 2    96-127  the bass learns to talk
    bridge    128-139 halftime; the machine idles
    build 3   140-147
    DROP 3    148-175 everything at once
    outro     176-191 back to the room
"""
import numpy as np
...
MID = dict(octave=24, spread=0.5, square=0.35, fold_=0.15, punch=1.0,
           transient=0.5, tilt=4.0, f_lo=220, f_hi=9500, res=3.2,
           drive=2.8, low=95, lfo_depth=0.65, smooth=0.008)
...
GAINS = {'drums': 0.57, 'sub': 0.235, 'bass': 0.26, 'texture': 0.42,
         'music': 1.90, 'pad': 1.30, 'atmos': 0.46, 'fx': 0.24}
s.report(GAINS)
s.render('neuro_bezdna_174.wav', drive=1.25, duck=0.34, limit=0.82, peak=0.90,
